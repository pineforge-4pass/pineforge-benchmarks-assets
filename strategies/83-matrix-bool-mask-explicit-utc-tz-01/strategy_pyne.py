...
from pynecore.lib import close, currency, dayofweek, hour, matrix, na, script, strategy, ta, time
from pynecore.types import Matrix, Persistent


@script.strategy("PF TA isolate 10 - typed-matrix UTC", shorttitle="TAI_10_TM_UTC", overlay=True, initial_capital=1000000, currency=currency.USD, commission_type=strategy.commission.percent, commission_value=0, slippage=0, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=1, process_orders_on_close=False)
def main():
