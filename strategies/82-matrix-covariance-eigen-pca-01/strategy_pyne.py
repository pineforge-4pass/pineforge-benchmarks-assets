...
def main(
    ...
    m__global__: Persistent[Matrix[float]] = matrix.new(2, 2, 0.0)
    matrix.set(m__global__, 0, 0, cov11)
    matrix.set(m__global__, 0, 1, cov12)
    matrix.set(m__global__, 1, 0, cov21)
    matrix.set(m__global__, 1, 1, cov22)

    covReady = not na(cov11) and (not na(cov12)) and (not na(cov22))

    lam: float = na(float)
    if covReady:
        lam = array.get(matrix.eigenvalues(m__global__), 0) if array.size(matrix.eigenvalues(m__global__)) > 0 else na
