...
"""
from pynecore.lib import close, currency, hour, minute, script, strategy, ta


@script.strategy("PF probe 91 - atr trail fixed entry", shorttitle="PF_P91_ATRTRAIL", overlay=True, initial_capital=1000000, currency=currency.USD, commission_type=strategy.commission.percent, commission_value=0, slippage=0, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=1, process_orders_on_close=False)
def main():
    atrLen: int = 14
    atrMult: float = 1.2
    atr = ta.atr(atrLen)
