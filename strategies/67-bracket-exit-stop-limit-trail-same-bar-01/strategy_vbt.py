...
def run_vbt(df: pd.DataFrame, fees: float = 0.001) -> vbt.Portfolio:
    ...
    # Calculate ATR using speed.vbt_helpers.atr
    atr_val = atr(df['high'], df['low'], df['close'], 14).fillna(0.0).values
    ...
    @njit
    def logic_loop(open_p, high_p, low_p, close_p, hours, minutes, atr_val, fees):
        ...
        for i in range(1, n):
            ...
            if pos == 0:
                if h_prev == 8 and m_prev == 15:
                    ...
                    # Note stop, limit, trail are set at the close of entry bar i
                    # So we use atr_val[i] instead of atr_val[i-1]
                    stop_price = entry_price - atr_val[i] * 0.8
                    take_price = entry_price + atr_val[i] * 1.6
                    trail_points = atr_val[i]
    ...
    entries, short_entries, exits, short_exits, exec_price = logic_loop(
        open_p, high_p, low_p, close_p, hours, minutes, atr_val, fees
    )
