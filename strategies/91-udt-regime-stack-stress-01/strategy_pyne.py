...
from pynecore.lib import (
    bar_index, barstate, close, color, currency, high, input, location, low,
    map, na, nz, open, plot, plotchar, script, size, strategy, ta
)
...
@script.strategy("UDT Regime Stack (PF stress)", overlay=True, initial_capital=1000000, currency=currency.USD, process_orders_on_close=False, pyramiding=1, commission_type=strategy.commission.percent, commission_value=0, slippage=0, default_qty_type=strategy.fixed, default_qty_value=1)
def main(
    ...
    gateMap__global__: Persistent[dict[str, float]] = map.new()
    if barstate.isfirst:
        map.put(gateMap__global__, 'long', 2.0)
        map.put(gateMap__global__, 'short', -2.0)
        map.put(gateMap__global__, 'exit_long', 0.0)
        map.put(gateMap__global__, 'exit_short', 0.0)

    __switch__ = profile
    if __switch__ == Aggression.conservative:
        thr_long = nz(map.get(gateMap__global__, 'long'), 2.0)
        thr_short = nz(map.get(gateMap__global__, 'short'), -2.0)
    ...
    gates.map_ok = map.contains(gateMap__global__, 'long') and map.contains(gateMap__global__, 'short')
    ...
    exitL = nz(map.get(gateMap__global__, 'exit_long'), 0.0)
    exitS = nz(map.get(gateMap__global__, 'exit_short'), 0.0)
