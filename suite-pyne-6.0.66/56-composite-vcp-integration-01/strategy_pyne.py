...
from pynecore.lib import (
    array, close, currency, high, input, low, math, na, open, script,
    strategy, ta, time, timeframe, volume
)
from pynecore.types import Persistent, Series


@script.strategy("VCP probe 08 - integration", shorttitle="VCP_p08_INT", overlay=True, initial_capital=1000000, currency=currency.USD, commission_type=strategy.commission.percent, commission_value=0, slippage=0, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=1, process_orders_on_close=False, max_boxes_count=300)
def main(
