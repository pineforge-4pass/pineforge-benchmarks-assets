...
@script.strategy("MTF probe 11 - HTF bracket manual trail", shorttitle="MTF_p11", overlay=True, initial_capital=1000000, commission_type=strategy.commission.percent, commission_value=0, slippage=0, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=1, process_orders_on_close=False)
def main(
    ...
    i_stop_mult=input.float(2.0, "Initial Stop ATR mult", minval=0.5, maxval=5.0, step=0.1),
    i_tp_mult=input.float(4.0, "Take-profit ATR mult", minval=0.5, maxval=10.0, step=0.1),
    i_trail_mult=input.float(2.0, "Manual Trail ATR mult", minval=0.5, maxval=5.0, step=0.1)
):
    ...
    if strategy.position_size > 0 and (not na(atrVal)):
        candidate: float = close - atrVal * i_trail_mult
        if not na(trailStop) and candidate > trailStop:
            trailStop = candidate
        strategy.exit('LX', 'L', stop=trailStop, limit=entryTP, comment='manual trail')
