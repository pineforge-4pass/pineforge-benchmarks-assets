...
from pynecore.lib import close, math, na, script, strategy, ta
...
class Stat:
    last: float = na(float)
    xMean: float = na(float)
    xSd: float = na(float)
    n: int = 0


@script.strategy("PF UDT probe 18 - method calls method", shorttitle="UDT_p18_M2M", overlay=True, initial_capital=1000000, commission_type=strategy.commission.percent, commission_value=0, slippage=0, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=1, process_orders_on_close=False)
def main():
    @method
    ...
    def zscore(self: Stat):
        m: float = mean(self)
        v2: float = variance(self)
        sd: float = math.sqrt(v2)
        return 0.0 if na(sd) or sd == 0.0 else (self.last - m) / sd
