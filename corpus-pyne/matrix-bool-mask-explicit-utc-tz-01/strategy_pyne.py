...
@script.strategy("PF TA isolate 10 - typed-matrix UTC", shorttitle="TAI_10_TM_UTC", overlay=True, initial_capital=1000000, commission_type=strategy.commission.percent, commission_value=0, slippage=0, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=1, process_orders_on_close=False)
def main():
    mask: Persistent[Matrix[bool]] = matrix.new(24, 7, False)
    rsiVal = ta.rsi(close, 14)

    h: int = hour(time, 'UTC')
    d: int = dayofweek(time, 'UTC') - 1

    if not na(rsiVal) and rsiVal > 60.0 and (h >= 0) and (h < 24) and (d >= 0) and (d < 7):
        matrix.set(mask, h, d, True)

    mT: Persistent[Matrix[bool]] = matrix.new(7, 24, False)
    mTT: Persistent[Matrix[bool]] = matrix.new(24, 7, False)
    mT = matrix.transpose(mask)
    mTT = matrix.transpose(mT)

    sample: bool = matrix.get(mTT, h if h < 24 else 0, d if d >= 0 and d < 7 else 0)
